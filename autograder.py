...
if __name__ == '__main__':
    ...
    print(
        f"### Pass condition: win rate >= {win_threshold/total_games*100:.1f}% ###\n")
    ...
    for i in range(games_per_side):
        print("\n" + "="*30)
        options = new_capture.readCommand(red_args)
        options['redTeamName'] = user_team_name
        options['blueTeamName'] = baseline_team
        games, timeList = new_capture.runGames(**options)
        if timeList == None:
        ...
        print(
            f"Progress: {len(game_summaries)}/{total_games} games finished. You won {win_count} times.", end='\r')
    ...
    for i in range(games_per_side):
        print("\n" + "="*30)
        options = new_capture.readCommand(blue_args)
    ...
    for res in game_summaries:
        ...
        print(
            f"{res['game_no']:<4} | {res['role']:<5} | {res['result']:<6} | {res['score']:<6} | {stats:<35}")
